...
class Hand {
  ...
  type(cards) {
    ...
    let tc = Object.keys(w)
    tc.sort((a, b) => w[b] - w[a])

    // TODO: we could maybe also sort matching counts (runs) 2nd by card type score

    // J cards act as wild cards which make hands the strongest possible type
    // so after our initial wordcount, we can simply add any Js to the highest, and then delete them from tc
    if (w['J']) {
      w[tc[0]] += w['J']
      tc = tc.filter(c => c != 'J')
    }
